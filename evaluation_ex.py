...
def calculate_ex(predicted_res, ground_truth_res, sql_idx=None, predicted_sql=None):
    res = 0
    if set(predicted_res) == set(ground_truth_res):
        res = 1
    else:
        print(f"MISMATCH at index {sql_idx}:")
        print(f"  SQL: {predicted_sql}")
        print(f"  Predicted: {predicted_res}")
        print(f"  Ground Truth: {ground_truth_res}")
        print(f"  Predicted set: {set(predicted_res)}")
        print(f"  Ground Truth set: {set(ground_truth_res)}\n\n")
    return res
...
def execute_model(
    ...
    try:
        # Create a wrapper function to pass additional parameters to calculate_ex
        def calculate_ex_with_context(pred_res, gt_res):
            return calculate_ex(pred_res, gt_res, sql_idx=idx, predicted_sql=predicted_sql)

        res = func_timeout(
            meta_time_out,
            execute_sql,
            args=(predicted_sql, ground_truth, db_place, sql_dialect, calculate_ex_with_context),
        )
...
def compute_acc_by_diff(exec_results, diff_json_path):
    print(f"exec_resuls: {exec_results}, diff_json_path: {diff_json_path}")
    ...
    for i, content in enumerate(contents):
        if exec_results[i]["res"] == 0:
            print(f"Failed {content["question_id"]}, index: {i}, difficulty: {content["difficulty"]}, {content["question"]}")
